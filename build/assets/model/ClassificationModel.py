...
def preprocess_image_deep(image):
    
    return image

data_traditional = np.array([preprocess_image_traditional(img) for img in data])
data_deep = np.array([preprocess_image_deep(img) for img in data])
...
# === Split Data ===
# Traditional ML
X_train, X_test, y_train, y_test = train_test_split(data_traditional, labels_encoded, test_size=0.2, shuffle=True, random_state=42)
# Deep Learning
X_train_dl, X_test_dl, y_train_dl, y_test_dl = train_test_split(data_deep, labels_encoded, test_size=0.2, shuffle=True, random_state=42)
